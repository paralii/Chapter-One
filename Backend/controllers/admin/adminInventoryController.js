...
export const updateProductStock = asyncHandler(async (req, res) => {
  try {
    ...
    if (!Number.isInteger(quantity) || quantity < 0) {
      return res.status(STATUS_CODES.CLIENT_ERROR.BAD_REQUEST).json({ success: false, message: 'Quantity must be a non-negative integer' });
    }
    if (!reason || typeof reason !== 'string' || reason.trim() === '') {
      return res.status(STATUS_CODES.CLIENT_ERROR.BAD_REQUEST).json({ success: false, message: 'Reason is required' });
    }
    ...
    res.status(STATUS_CODES.SUCCESS.OK).json({ success: true, message: 'Stock updated successfully', product });
  } catch (err) {
    res.status(STATUS_CODES.SERVER_ERROR.INTERNAL_SERVER_ERROR).json({ success: false, message: 'Error updating stock', error: err.message });
  }
});
